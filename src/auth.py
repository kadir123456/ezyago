...
class AuthManager:
    ...
    async def register_user(self, email: str, password: str, full_name: str) -> Optional[UserData]:
        try:
            ...
            user_data_dict = {
                "uid": firebase_user.uid, "email": email, "password_hash": password_hash,
                ...
                "bot_status": "stopped", "total_trades": 0, "winning_trades": 0,
...
async def get_active_user(current_user: UserData = Depends(get_current_user)) -> UserData:
    # --- DÜZELTİLEN BÖLÜM ---
    now = datetime.now(timezone.utc) # UTC'li (aware) zaman damgası kullanıyoruz
    is_expired = False
    ...
    if current_user.subscription_status == SubscriptionStatus.TRIAL:
        if current_user.trial_end_date and current_user.trial_end_date <= now:
            is_expired = True
    elif current_user.subscription_status == SubscriptionStatus.ACTIVE:
    ...
    if is_expired:
        ...
        raise HTTPException(
            ...
            detail="Aboneliğinizin süresi doldu. Lütfen yenileyin."
        )
